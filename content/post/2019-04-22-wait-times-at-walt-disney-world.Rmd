...
```
Now we train an xgboost model on the training set. I use 200 rounds of boosting. The "early_stop_round" hyper-parameter (set to 20) specifies the number of rounds after which the algorithm will stop if performance has not improved.
...
We can conduct hyper-parameter tuning to improve performance. I will use the mlr package for tuning. Unlike xgboost, which requires a matrix, mlr functions take in a dataframe as input. So I will use the mrides_long df again.
```{r,message=FALSE}
library(mlr)
library(caret)
...
xgb_model <- mlr::train(xgb_tuned_param, train_task)
