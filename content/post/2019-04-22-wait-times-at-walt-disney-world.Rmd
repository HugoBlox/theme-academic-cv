...
```
The Hyperparameter results from tuning are shown above. Now, we can re-train our model with these results. 
```{r}
...
```
Our performance improved significantly with hyper-parameter tuning. This model performs better than the one created with the xgboost package (although we did not tune that model).
