...
Next, using the tidyverse list-column workflow, we can build a separate model for each ride within the same dataframe.  
```{r}
rides_nest<-mrides %>% gather(key="ride",value="wait_time",toy_story_mania:soarin)%>%  group_by(ride) %>% nest()
head(rides_nest)
```
The data column is a <list> where each element is a tibble that stores the data corresponding to the ride variable. 
