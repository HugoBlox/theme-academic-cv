...
```
To tokenize the review text into a sequence of integers; each word will correspond to an integer. To get the 1,000 most frequent words in our text, we set num_words to 1,000.
```{r}
...
plot(hist)
```
The training loss decreases with each epoch, while the accuracy increases. However, the validation data does not show this same trend, which indicates that we are overfitting to training data. 
```{r}
hist
```
