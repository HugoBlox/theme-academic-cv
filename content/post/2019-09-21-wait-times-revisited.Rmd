...
files %>% filter(ride=="slinky_dog" & hour(datetime)==20) %>% tail()
```
Remove times from 1am-5am, as these are incorrent entries.
```{r}
files<-files %>% 
  mutate(
    hour=hour(datetime)) %>% 
...
fp_ts %>% filter(ride=="slinky_dog" & date(date)==ymd("2019-08-31")) %>% as.data.frame()
...
Overall comparison 
```{r}
p1<-fp_ts %>% ggplot(aes(x=date,y=avgTime)) + 
  geom_line()+
